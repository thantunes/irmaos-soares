...
const CalculadoraDePisos: StorefrontFunctionComponent<CalculatorProps> = () => {
    ...
    const checkCategory = () => {
        ...
        if (categories) {
            if (categories.includes("/Pisos e Revestimentos/")) {
                ...
                if (sellingPrice) {
                    document.querySelector(".vtex-product-price-1-x-sellingPrice")?.remove();
                }
            } else {
    ...
    const pricePerSquareMeter = product?.items[0].sellers[0].commertialOffer.Price!;
    ...
    return (
        <>
            <section className={classNames.wrapper}>
                <h3 className={classNames.title}>Quantos metros quadrados você precisa?</h3>
                <div className={classNames.calculadora_box}>
                    ...
                    <div className={classNames.right}>
                        ...
                        <span className={classNames.countBoxesPrice}>{formatPrice(countBoxesPrice)}</span>
                    </div>

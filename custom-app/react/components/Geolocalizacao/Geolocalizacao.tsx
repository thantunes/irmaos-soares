...
function Geolocalizacao() {
  ...
  async function getPolicyForPostalCode(postalCode: string) {
    ...
    const response = await fetch(
      `/api/dataentities/CP/search?_fields=CepFinal,IdPoliticaComercial,CepInicial,NomePoliticaComercial&_where=${where}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  async function handleCepChange(cep: string) {
    try {
      const policy = await getPolicyForPostalCode(cep)
        .then((policy) => policy)
        ...
        <div className="flex flex-column flex-row-ns">
          <div className="w-100 w-100-ns flex justify-start ">
            <div className="w-100 mb4">
              <p className="mb3" id="modal-description">
                Diferentes opções de entrega e ofertas para sua cidade.
              </p>
              <InputMask
                mask="99999-999"
                value={cep}
              ...
              >
                <Input
                  ...
                  helpText={
                    'Ao informar, você concorda com a nossa Política de Privacidade.'
                  }
                  value={cep}
                />
              </InputMask>
              <a
                ...
                className="modal-localizacao__link c-link mv3"
              >
                Não sei meu CEP
              </a>
            </div>

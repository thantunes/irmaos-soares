            ...
            {lojas.map((shop: Loja, index: number) => {
                return (
                    <div className={styles.store} key={index}>
                        <div>
                            <img className="w-100" src={shop.photoStore} alt={shop.nameStore} />
                        </div>
                        <div className={styles.shopInfo}>
                            ...
                            <a
                                ...
                                target="_blank"
                                className={styles.phoneStore}>{shop?.phoneStore}</a>
                        </div>
                        <div className={styles.addressContent}>
                            <p className={styles.address}>{shop?.addressStore}</p>
                            {shop.linkStore ? <a href={shop.linkStore} className={styles.storeLink} target="_blank" rel="noopener noreferrer">Ver no mapa</a> : null}
                        </div>
                    </div>
                )
            })}
